 ...
 */
const getPolitical_parties = async (): Promise<void> => {
  
  // Realizamos la consulta: 
   //.from('political_parties') -> Selecciona la tabla de tu imagen.
  ...
  if (error) {
    console.error("❌ Error al obtener los Political_parties:", error.message);
    return;
  }

  // Si todo sale bien, 'data' contiene el array de objetos.
  // Usamos 'as Auto[]' para decirle a TS que confíe en nuestra interfaz.
  const political_parties: political_parties[] = data as Political_parties[];

  // Mostramos el resultado final en la consola del navegador
  console.log("✅ Lista de Political_parties recibida:");
  console.table(political_parties); 
