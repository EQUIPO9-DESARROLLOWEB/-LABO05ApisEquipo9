...
const getcandidates = async (): Promise<void> => {
  ...
  if (error) {
    console.error("❌ Error al obtener los candidates:", error.message);
    return;
  }

  // Si todo sale bien, 'data' contiene el array de objetos.
  // Usamos 'as Auto[]' para decirle a TS que confíe en nuestra interfaz.
  const listacandidates: candidates[] = data as candidates[];

  // Mostramos el resultado final en la consola del navegador
  console.log("✅ Lista de candidates recibida:");
  console.table(listacandidates); 
